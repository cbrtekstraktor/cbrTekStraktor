...
public class cmcOCRController {
	...
	// watch out - there is a copyLineItem method
	class OCRLineItem
	{
		int GraphObjIdx=-1;
		int AbsX=-1;
	...
	//---------------------------------------------------------------------------------
	{
		...
		Font ocrfont = new Font("arial", Font.PLAIN, ocrfonthoogte );
		// text
		...
		for(int i=0;i<list.size();i++)
		{
		 //process paragraph
		 OCRParagraph para = list.get(i);
		 String sTag = preamble.trim() + (""+list.get(i).UID).trim() + postamble.trim();
...
//drawRectangle( leftMarginWidth , prevY , canvasBreedte , yOffset-prevY , imgWidth);
	...
	//---------------------------------------------------------------------------------
	{
		...
		for(int i=0;i<x.ar_item.length;i++)
		{
			...
			//x.ar_item[i].RelY = x.ar_item[i].AbsY - x.MaxY;  // opted to align at the BOTTOM
			x.ar_item[i].RelY = x.ar_item[i].AbsY - x.MinY;
		}
		// relatieve afstanden - sort on the absX
		boolean swap = false;
	...
	//---------------------------------------------------------------------------------
	public boolean run_tesseract()
	//---------------------------------------------------------------------------------
	{
		// TES - PATH en options - taal zetten
	    ...
	    //	
		if( createTesseractCommandFile() == false ) {
	...
	//---------------------------------------------------------------------------------
	private boolean createTesseractCommandFile()
	//---------------------------------------------------------------------------------
	{
        ...
        {
        case MSDOS : return createTesseractMSDOSBatchFile(TessLanguage);
        case LINUX : return createTesseractLinuxShellFile(TessLanguage);
	...
	//---------------------------------------------------------------------------------
	private boolean createTesseractLinuxShellFile(String TessLanguage)
	//---------------------------------------------------------------------------------
	{
		 do_error("Linux version Tesseract Under Construction"); return false;
	}

	//---------------------------------------------------------------------------------
	private boolean createTesseractMSDOSBatchFile(String TessLanguage)
	//---------------------------------------------------------------------------------
	{
	...
	//---------------------------------------------------------------------------------
	{
		for(int i=0;i<list.size();i++)
		{
		 ...
		 int p = sIn.indexOf( (""+para.UID).trim());
		 if( p >= 0 ) return para.UID;
		}
		return -1L;
	}
	//---------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------
	{
    	...
    	for(int i=0;i<=aantal;i++)
    	{
    		...
    		if( currentIdx == -1 ) {  // quality statement
    			int q= determineQuality(sLijn);
    			do_log(1,"Maximum Quality Indicator [" + q + "%]");
    		    continue;	
    		}
    	...
    	int unres=0;
    	for(int i=0;i<list.size();i++) if( list.get(i).ingested == false ) unres++;
